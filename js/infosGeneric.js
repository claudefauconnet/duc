var infosGeneric = (function () {
    ...
    self.setAttributesValue = function (collection, obj, attrObject) {
        ...
        for (var key in attrObject) {
            ...
            var selectFields = infosGenericParams.fieldsSelectValues[collection];
            if (selectFields) {
            ...
            if (type && type == 'select' && selectValues) {
                var str = "<select onchange='infosGeneric.incrementChanges();' class='objAttrInput' id='attr_" + key + "'>"
                ...
                value = "<input type='password' onchange='infosGeneric.incrementChanges();' class='objAttrInput' " + strCols + "id='attr_"
                    + key + "' value='" + value + "'>";
            }
                ...
                if (rows) {// textarea
                    if (cols)
                        strCols = " cols='" + cols + "' ";
                    rows = " rows='" + rows + "' ";
                    value = "<textArea  onchange='infosGeneric.incrementChanges();' class='objAttrInput' " + strCols + rows
                        + "id='attr_" + key + "' >" + value + "</textarea>";
                } else {
                    if (cols)
                        strCols = " size='" + cols + "' ";
                    value = "<input onchange='infosGeneric.incrementChanges();' class='objAttrInput' " + strCols + "id='attr_"
                        + key + "' value='" + value + "'>";
                }
            }
    ...
    self.save = function (dbName, collection, obj, display, additionalAttrsObj) {
        infosGeneric.isModifying=false;
